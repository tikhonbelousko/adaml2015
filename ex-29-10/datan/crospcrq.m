  function [Q2,Q2y,ypred,press,pressy]=crospcrq(x,y,dims,part,iscale,iperm);
% keywords: variable selection, regressio, crossvalidation, principle components
% call: [Q2,Q2y,ypred,press,pressy]=crospcrq(x,y,dims,part,iscale,iperm);
% The function calculates the value of the crossvalidation criterion
% Q2,press for the dimensions 'dims' using the PCR-method. A scaling
% of 'x' is optional. The data is devided in 'part' parts. 'Press' is calculated by
...
% INPUT  :      x,y       data matrices   [m,ny] = size(y)
...
for i = 1:imax;
     ...
     for j = 1:ndim;
        ...
        if iscale == 1
            ...
            ypred(ind,indy) = pcrpred(xipred,p,q,dim,yimean);
        else
            xipred = x(ind,:);
            ypred(ind,indy) = pcrpred(xipred,p,q,dim,yimean,ximean);
        end
     end
end
